...
<script type="text/javascript">
    ...
    function add(){
        ...
        if(!validate){
            $.messager.alert("消息提示","请检查你输入的数据！","warning");
            return ;
        }
    ...
    //某个角色拥有的角色
    var existsAuthority =null;
    function isAdded(row,rows) {
        for(var k=0;k<existsAuthority.length;k++){
             if(existsAuthority[k].menuId == row.id && haveParent(rows,row.parentId)) return true;
    ...
    function convert(rows){
        ...
        function selectAuthority(roleId) {
            $('#select-authority-dialog').dialog({
                ...
                onBeforeOpen: function () {
                    $.ajax({
                        url:'get_roleAuthority',
                        type:"post",
                        data:{roleId:roleId},
                        dataType:'json',
                        success:function(data){
                             existsAuthority=data;
                            $("#authority-tree").tree({
